...
const ClientCart = {
    ...
    data: [],
    Add: function (sessionid, produto_id, produto_titulo, qtde) 
    {
        
        if(ClientCart.SizeOf()>0)
        {
            for(var i = 0; i< ClientCart.SizeOf(); i++) {
                if(ClientCart.data[i].pid !== produto_id){
                    ClientCart.data.push( { sessionid: sessionid, pid : produto_id , nome: produto_titulo, qtde : qtde } )
                    break;
                }
            }
        }
        ...
        Armazenamento.Set("cart_list",ClientCart.data)
        ClientCart.UpdCounter()
        ClientCart.Persistence("./add-cart")
        
    },
    Del: function (produto_id){
        
        if(ClientCar.SizeOf() > 0)
        {
            for(var i = 0; i < ClientCar.SizeOf(); i++) {
                if(ClientCar.data[i].pid === produto_id){
                    ClientCar.data.splice(i, 1);
                    break;
                }
            }
        }
    ...
    Persistence: function(url)
    {
        $.ajax({
            type: "POST", url: url, data: { item: JSON.parse( Armazenamento.Get("cart_list") ) },
            success: function(rs, textoStatus, xhr){ console.log('Sucesso --> ', url , rs, textoStatus) },
            error: function(rs, status, err){ console.log('Erro --> ',rs , url, err) }
        });
    },
    SizeOf: function(){
        return (localStorage.cart_list===undefined)?0:ClientCart.data.length;
    },
    SetItens: function ()
    {
        if(localStorage.cart_list!=undefined) {
            ClientCart.data = JSON.parse(Armazenamento.Get("cart_list"))
        }
        ClientCart.UpdCounter()
    },
    UpdCounter: function()
    {
        $("span#badge-tot-cart-list1").html(ClientCart.SizeOf())  
        if($("span#badge-tot-cart-list2")){
            $("span#badge-tot-cart-list2").html(ClientCart.SizeOf())  
        }
    }
...
const WishList = {
    ...
    data: [],
    total: 0.00,
    Add: function (produto_id, produto_titulo, preco) 
    {
        if(WishList.SizeOf()>0)
        {
            for(var i = 0; i< WishList.SizeOf(); i++) {
                if(WishList.data[i].pid !== produto_id){
                    WishList.data.push( { pid : produto_id , nome: produto_titulo, preco : preco } )
                    break;
                }
            }
        }
        ...
        Armazenamento.Set("whish_list",WishList.data)
        WishList.UpdCounter()
    ...
    SetItens: function ()
    {
        ...
        if(localStorage.whish_list!=undefined) {
            WishList.data = JSON.parse(Armazenamento.Get("whish_list"))
            WishList.total = 0.00;
            for(var i=0; i<WishList.SizeOf(); i++) {
                if(document.getElementById(WishList.data[i].pid)){
                    document.getElementById(WishList.data[i].pid).setAttribute("class","wishlist atw-demo active")
                }
            }
    ...
    SizeOf: function() {
        return (localStorage.whish_list===undefined)?0:WishList.data.length;
    },
    Persistence: function(url)
    {
        $.ajax({
            type: "POST", url: url, data: { item: JSON.parse( Armazenamento.Get("whish_list") ) },
            success: function(rs, textoStatus, xhr){ console.log('Sucesso --> ', url , rs, textoStatus) },
            error: function(req, status, err){ console.log('Erro --> ', url, err) }
        });
    },
    UpdCounter: function()
    {
        $("span#badge-tot-whish-list1").html(WishList.SizeOf())
        if($("span#badge-tot-whish-list2")){
            $("span#badge-tot-whish-list2").html(WishList.SizeOf())
        }
    }
...
/*
atcDemo: function() { //ALERT ADICIONAR AO CARRINHO - ADD TO CART
    $(".atc-demo").click(function(e) {
        ...
        //ClientCart.Add(produto_id, n, preco)
        
        new Noty({
            type: "success",
            text: '<div class="media"><i data-feather="check-circle"></i><div class="media-body ml-3"><strong>' + t + "</strong><br/>adicionado no CARRINHO DE COMPRAS!</div></div>",
            timeout: 2e3
        }).show(), feather.replace(), e.preventDefault()
    })
},*/
atwDemo: function() { //ALERT LISTA DE DESEJOS - WISH LIST
    $(".atw-demo").click(function(e) {
        var t = $(this),
            produto_id = t.attr("id"),
            preco = t.attr("data-value"),
            n = t.closest(".card").find(".card-title").text(),
            s = t.hasClass("active") ? "Removido de" : "Adicionado em sua",
            tipoAlert = t.hasClass("active") ? "pink" : "success";
            if(tipoAlert=="success") {
                WishList.Add(produto_id, n, preco)
            }else{
                WishList.Del(produto_id)
            }

        new Noty({
            type: tipoAlert,
            text: '<div class="media"><i data-feather="heart"></i><div class="media-body ml-3"><strong>' + n + "</strong><br/>" + s + " lista de desejos.</div></div>",
            timeout: 3e3
        }).show(), feather.replace(), t.hasClass("active") ? t.attr("title", "Adicionar na Lista de Desejos") : t.attr("title", "Adicionado na Lista de Desejos"), t.hasClass("active") ? t.removeClass("active") : t.addClass("active"), t.trigger("mouseout"), e.preventDefault()
    })
},
